...
export const createMockEvents = (): PanelEventEmitter & {
  ...
  return {
    emit: (eventTypeOrEvent: string | PanelEvent<unknown>, payload?: unknown) => {
      // Support both (event) and (eventType, payload) signatures
      let event: PanelEvent<unknown>;
      if (typeof eventTypeOrEvent === 'string') {
        event = {
          type: eventTypeOrEvent as PanelEventType,
          payload,
          source: 'mock',
          timestamp: Date.now(),
        };
      ...
      // eslint-disable-next-line no-console
      console.log('[Mock] Emitting event:', event.type, event.payload);
...
/**
 * Create mock panel context for use in stories
 */
